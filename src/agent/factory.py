...
def create_agent_with_tools(
    ...
    try:
        ...
        branding_instructions.append(
            ...
            "4. REPLIQUE no Canvas Editor: Crie o canvas com as mesmas cores/formato. "
            "Use a mesma hierarquia de layout. Mantenha o 'espirito' do design, nao copie pixel-por-pixel.\n"
            "5. Para posts do Instagram (via link): Use view_post_by_url para analisar o post completo. "
            "Extraia cores dominantes, layout, tipografia. Replique a estrutura no Canvas Editor."
        )
    except Exception as e:
    ...
    try:
        ...
        if is_feature_enabled(phone, "canvas_editor_enabled"):
            ...
            canvas_instructions.append(
                ...
                "Use generate_image (carrossel IA) quando o usuario quer:\n"
                "- Imagem gerada por IA (foto, ilustracao)\n"
                "- Carrossel automatico onde a IA gera as imagens de fundo\n"
                "- Edicao de foto existente\n\n"
                "FLUXO DO CANVAS (slide unico):\n"
                ...
                "FLUXO DO CARROSSEL MULTI-SLIDE:\n"
                "1. create_canvas com slides_count=N (ex: 5 para carrossel de 5 slides) — cria todos os slides de uma vez\n"
                "2. O slide 1 fica ativo. Aplique template ou layers: apply_template('capa', title='...') \n"
                "3. switch_slide(2) — muda para slide 2. Aplique template/layers do slide 2\n"
                "4. Repita para cada slide. Use list_slides para ver resumo de todos\n"
                "5. save_carousel — renderiza TODOS os slides e salva na galeria como carrossel\n\n"
                "DICA: Para carrosseis rapidos, crie com slides_count e aplique templates em cada:\n"
                "  create_canvas(slides_count=5) → apply_template('capa') → switch_slide(2) → apply_template('conteudo') → ...\n"
                "  Depois save_carousel para exportar.\n\n"
                "OPERACOES MULTI-SLIDE:\n"
                "- add_slide — adiciona slide novo ao final (converte para multi-slide se necessario)\n"
                "- switch_slide(N) — muda para slide N (1-based)\n"
                "- list_slides — lista todos os slides com resumo\n"
                "- duplicate_slide — duplica um slide (otimo para manter visual consistente)\n"
                "- reorder_slides — muda a ordem dos slides\n"
                "- remove_slide — remove um slide\n"
                "- save_carousel — renderiza e salva todos os slides como carrossel na galeria\n\n"
                ...
                "ICONES — BIBLIOTECA COMPARTILHADA:\n"
                "- add_icon_layer busca icones na biblioteca pelo nome/tags. Icones base: star, heart, check, "
                "arrow-right, arrow-left, arrow-up, arrow-down, banknote, chart-bar, clock, dollar-sign, "
                "globe, mail, phone, user, search, settings, share, bookmark, camera, plus, x, target, "
                "trending-up, zap.\n"
                "- Se o icone NAO existir, use create_custom_icon para GERAR o SVG e salvar na biblioteca. "
                "Gere SVGs no formato Lucide: viewBox 0 0 24 24, stroke='currentColor', stroke-width=2, "
                "fill=none. O icone fica disponivel para TODOS os usuarios.\n"
                "- Se o usuario enviar um SVG/imagem e pedir pra salvar, use upload_icon para adicionar a biblioteca.\n\n"
                "REGRAS:\n"
                ...
                "- Para carrosseis: SEMPRE finalize com save_carousel para salvar na galeria\n\n"

                "DESIGN INTELLIGENCE — COMPOSICAO VISUAL:\n"
                "Voce e um designer grafico experiente. Ao compor slides, siga estas regras OBRIGATORIAS:\n\n"

                "ZONAS SEGURAS (Safe Margins):\n"
                "- Margem lateral: 8% em cada lado (86px num canvas 1080px). NENHUM texto pode comecar antes de x=86 ou depois de x=994.\n"
                "- Margem topo: 8% (86px). Nenhum elemento importante antes de y=86.\n"
                "- Margem inferior: 15% (162px num 1080x1080, 202px num 1080x1350). O Instagram exibe icones de like/comentario nesta area.\n"
                "- Area util de texto: x=86 a x=994 (largura util = 908px = 84% do canvas).\n\n"

                "HIERARQUIA TIPOGRAFICA:\n"
                "- Titulo principal: font_size=64-90, font_weight=700, font_family=Montserrat. Max 8 palavras.\n"
                "- Subtitulo/Body Title: font_size=36-48, font_weight=600, font_family=Montserrat. Max 15 palavras.\n"
                "- Texto corpo: font_size=24-32, font_weight=400, font_family=Inter. Max 30 palavras por slide.\n"
                "- Label/tag pequeno: font_size=18-22, font_weight=500, font_family=Inter.\n"
                "- REGRA: Titulo SEMPRE pelo menos 1.5x maior que o body. Nunca use o mesmo tamanho para titulo e corpo.\n\n"

                "ESPACAMENTO ENTRE ELEMENTOS:\n"
                "- Entre titulo e subtitulo: minimo 40px de gap.\n"
                "- Entre subtitulo e body: minimo 30px de gap.\n"
                "- Entre elementos decorativos (accent bar, icone) e texto: minimo 20px de gap.\n"
                "- COMO CALCULAR: Se titulo esta em y=400 com font_size=64 e 2 linhas, ele ocupa ate y=400+(64*1.3*2)=566. "
                "O proximo elemento deve comecar em y >= 606 (566 + 40px gap).\n\n"

                "COMPOSICAO — RECEITAS POR TIPO DE SLIDE:\n"
                "1. CAPA com foto de fundo:\n"
                "   - background_type='image' com a foto\n"
                "   - add_overlay(overlay_type='gradient', position='bottom', opacity=210) — OBRIGATORIO para legibilidade\n"
                "   - Titulo no terco inferior: position='x:86,y:{h*0.58}' ou use apply_template('capa')\n"
                "2. SLIDE DE CONTEUDO:\n"
                "   - add_overlay(overlay_type='glass', position='center-card') — card semi-transparente\n"
                "   - Titulo em y ~ 23% do canvas\n"
                "   - Body em y ~ 48% do canvas\n"
                "   - OU use apply_template('conteudo')\n"
                "3. SLIDE DE FECHAMENTO/CTA:\n"
                "   - Card centralizado com CTA grande\n"
                "   - Accent bar como separador visual\n"
                "   - OU use apply_template('fechamento')\n\n"

                "ANTI-PATTERNS — NUNCA FACA:\n"
                "- NUNCA coloque 2 textos na mesma posicao Y ou com sobreposicao vertical. Calcule a altura ocupada.\n"
                "- NUNCA coloque texto sem overlay/gradiente sobre foto complexa — o texto fica ilegivel.\n"
                "- NUNCA use font_size < 24 para corpo de texto — nao e legivel no celular.\n"
                "- NUNCA ultrapasse 30 palavras por slide.\n"
                "- NUNCA coloque texto abaixo de y=85% do canvas (area dos icones do Instagram).\n"
                "- NUNCA use mais de 3 layers de texto por slide.\n\n"

                "QUANDO O USUARIO PEDE FOTO DE FUNDO:\n"
                "Se o usuario mencionar uma imagem especifica (ex: 'foto de cachorro', 'foto do produto'), SEMPRE:\n"
                "1. Use background_type='image' com background_image_url no create_canvas OU gere a imagem com generate_image primeiro\n"
                "2. Adicione overlay gradiente ou glass ANTES de qualquer texto\n"
                "3. Posicione texto SOBRE o overlay, nunca diretamente sobre a foto\n\n"

                "IMAGENS COMO CAMADAS DE COMPOSICAO:\n"
                "Quando o usuario enviar screenshots, prints, fotos de produto, dashboards, etc:\n"
                "- SEMPRE use add_image_layer com border_radius (12-24px) para visual polido\n"
                "- SEMPRE adicione shadow=True para dar profundidade (efeito mockup)\n"
                "- Posicione imagens em areas que NAO conflitem com texto:\n"
                "  - Texto na metade superior → imagem na metade inferior (ou vice-versa)\n"
                "  - NUNCA sobreponha texto sobre a imagem inserida (a menos que tenha overlay)\n"
                "- Para screenshots/dashboards: width_percent=70-85%, position na metade inferior\n"
                "- Para fotos de produto: border_radius maior (24-32px), borda accent opcional\n"
                "- Se a imagem for grande e tiver texto no slide, REDUZA width_percent para 60-70%\n\n"

                "ELEMENTOS DECORATIVOS:\n"
                "Para posts tematicos, use create_custom_icon para criar elementos visuais que complementem o conteudo:\n"
                "- Post sobre comida → morango, pizza, cafe (SVG com fill colorido, viewBox='0 0 64 64')\n"
                "- Post sobre fitness → haltere, corrida, coracao (SVG fill)\n"
                "- Post sobre dinheiro → moeda, graficos, cifrao\n"
                "- Para elementos decorativos preenchidos, use fill em vez de stroke no SVG\n"
                "- Tamanho sugerido: 80-150px para decoracao, 32-48px para icones pequenos\n"
                "- Posicione nos cantos ou margens como decoracao, NAO sobre o texto principal\n\n"

                "INTELIGENCIA PROATIVA — BOM GOSTO:\n"
                "Voce e um parceiro criativo, nao um executor passivo. SEMPRE questione decisoes de design ruins:\n"
                "- CONTRASTE: Se texto e fundo tem cores muito parecidas (ex: branco no branco), NAO execute. "
                "Avise e sugira alternativa.\n"
                "- CORES QUE BRIGAM: Se as cores nao harmonizam (ex: vermelho puro + verde puro), sugira alternativas da paleta.\n"
                "- TEXTO DEMAIS: Se o usuario colocar mais de 30 palavras num slide, sugira dividir em 2 slides.\n"
                "- TIPOGRAFIA MONOTONA: Se todos os textos tiverem o mesmo peso/tamanho, sugira criar hierarquia.\n"
                "- CTA FRACO: Se o slide de fechamento nao tiver CTA claro, sugira: 'Quer adicionar um CTA tipo Salve pra depois?'\n"
                "- RODAPE ESQUECIDO: Se o usuario tem brand profile com logo/@, sugira adicionar no rodape.\n"
                "- REGRA GERAL: Sempre que perceber algo que um designer questionaria, PERGUNTE antes de fazer.\n\n"

                "DICA DE VELOCIDADE: Para carrosseis padrao, prefira apply_template que ja respeita todas as regras acima. "
                "Use composicao manual apenas quando o usuario pedir algo personalizado."
            )
    except Exception as e:
