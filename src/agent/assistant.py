...
def get_assistant(session_id: str, extra_tools: list = None, channel: str = "whatsapp", extra_instructions: list = None, include_scheduler: bool = True, include_knowledge: bool = True, user_id: str = None) -> Agent:
    ...
    base_instructions = [
        ...
        "\nPESQUISA WEB — REGRAS CRITICAS:"
        f"\n- SEMPRE inclua o ano atual ({_now_br.year}) nas queries para evitar resultados desatualizados."
        "\n  Errado: 'Claude Opus modelo Anthropic'"
        f"\n  Certo: 'Claude Opus {_now_br.year} Anthropic novo modelo'"
        "\n- Para NOTICIAS RECENTES: use topic='news' e days=7 para pegar so as ultimas noticias."
        "\n- Faca MULTIPLAS buscas complementares com queries variadas quando o assunto e complexo."
        "\n- Se os resultados parecem desatualizados, refaca a busca com termos mais especificos e o ano.",
        # Geração de imagens — 2 fluxos: IA Pura + HTML Carousel
        "GERACAO DE IMAGENS (generate_image): Dois fluxos disponíveis."
        "\n"
        "\nFLUXO 1 — IMAGEM IA PURA (1 ou N imagens):"
        "\n- Para gerar imagens com IA (fotos, ilustracoes, arte digital)."
        "\n- Suporta single (1 slide) ou batch (N slides com temas diferentes)."
        "\n- Para EDITAR foto existente: reference_source='original' ou 'last_generated'."
        "\n"
        "\nFLUXO 2 — CARROSSEL HTML (1 ou N slides):"
        "\n- Para carrosseis com design consistente e pixel-perfect."
        "\n- O LLM gera HTML/CSS do zero baseado em referencia visual."
        "\n- Ideal quando o usuario quer design profissional com texto."
        "\n- Use generation_mode='html'."
        "\n"
        "\nFLUXO DE COLETA DE CONTEXTO (AMBOS os fluxos):"
        "\n  1. ENTENDA o pedido. Se vago, pergunte:"
        "\n     - Pra que vai usar? (post Instagram, banner, thumbnail)"
        "\n     - Tem alguma referencia visual? (print, link, estilo)"
        "\n     - Algum estilo especifico? (foto real, ilustracao, 3D, minimalista)"
        "\n     - Quantas imagens/slides?"
        "\n  2. Chame generate_image COM skip_refinement=False (padrao)."
        "\n     O Content Strategist vai refinar o conteudo e retornar um JSON."
        "\n  3. MOSTRE o resultado do refinamento ao usuario:"
        "\n     'Refinei o conteudo: [resumo das mudancas]. Posso gerar?'"
        "\n  4. Se CARROSSEL HTML: pergunte por referencia visual."
        "\n     'Manda uma referencia visual (print ou link) ou quer que eu crie um design?'"
        "\n  5. Apos aprovacao, chame generate_image NOVAMENTE com:"
        "\n     - slides=refined_slides (do JSON retornado)"
        "\n     - skip_refinement=True"
        "\n     - reference_image_url se o usuario mandou referencia"
        "\n     - generation_mode conforme escolhido"
        "\n"
        "\nCAMPOS DO SLIDE:"
        "\n- 'role': 'capa', 'conteudo' ou 'fechamento'"
        "\n- 'prompt': descricao da imagem (fundo limpo se tiver overlay de texto)"
        "\n- 'title': titulo/headline (max 50 chars)"
        "\n- 'body': texto complementar (max 120 chars)"
        "\n- 'cta_text': texto do CTA (APENAS no fechamento)"
        "\n"
        "\nPARAMETROS NOVOS:"
        "\n- generation_mode: 'auto' (padrao), 'ai' (forca IA pura), 'html' (forca HTML carousel)"
        "\n- reference_image_url: URL da referencia visual para design"
        "\n- skip_refinement: True para pular Content Strategist (usar apos aprovacao)"
        "\n"
        "\nREGRAS:"
        "\n- SEMPRE 1 unica chamada por pedido. '3 imagens' = 1 chamada com 3 slides."
        "\n- Para carrossel: generation_mode='html' (padrao via auto)."
        "\n- Para imagem unica: generation_mode='ai' (padrao via auto)."
        "\n- NUNCA preencha delivery_channel por conta propria."
        "\n- O modo 'auto' decide: carousel→html, single→ai.",
